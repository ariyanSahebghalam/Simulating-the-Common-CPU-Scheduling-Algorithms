// Priority scheduler
// From what I understand, we have to create a list of tasks, schedule them, but according to the priority of the job first.
// For me to do that, I have to make the scheduler
// pick the highest priority task, delete, then the next priority task, repeat. Has to be stable.
...
Task *pickNextTask() {
    ...
    Task *high_priorityT = (*list_head)->task;


    while (pointer->next != NULL) {
        pointer = pointer->next;

        if (pointer->task->priority >= high_priorityT->priority) {
            high_priorityT = pointer->task;
        }
    }


    // Must constantly remove the last task, till no task remains, which ends in scheduler
    delete(list_head, high_priorityT);
    return high_priorityT;
}