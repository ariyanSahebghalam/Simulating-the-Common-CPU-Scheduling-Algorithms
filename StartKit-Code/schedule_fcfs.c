...
// We have to implement the add function first
...
void add(char *name, int priority, int burst) {
    ...
    // list.c, creates a new node and adds the node, with the newly created task at the head
...
void schedule() {
    while (*list_head != NULL) {
        Task *executing_task = pickNextTask();          // picks the task to be executed
...
Task *pickNextTask() {
    ...
    // Note list_head after all the insertions, points to the first task in the list, which is the last in the text file, which is why our pointer goes all the way to the end
